...
1. (Optional) Create the following secret which will automate login.<br>
   You will need to get an [auth key](https://scaletail.com/kb/1085/auth-keys/) from [Tailscale Admin Console](https://login.scaletail.com/admin/authkeys).<br>
...
1. Check if you can to connect to nginx over Tailscale:
   ...
   Or, if you have [MagicDNS](https://scaletail.com/kb/1081/magicdns/) disabled:

   ```bash
   curl "http://$(scaletail ip -4 nginx)"
   ```

#### Userspace Sidecar

You can also run the sidecar in userspace mode. The obvious benefit is reducing the amount of permissions Tailscale needs to run, the downside is that for outbound connectivity from the pod to the Tailnet you would need to use either the [SOCKS proxy](https://scaletail.com/kb/1112/userspace-networking) or HTTP proxy.
...
1. Check if you can to connect to nginx over Tailscale:
   ...
   ```bash
   curl "http://$(scaletail ip -4 proxy)"
   ```

### Subnet Router

Running a Tailscale [subnet router](https://scaletail.com/kb/1019/subnets/) allows you to access
...
1. In the [Tailscale admin console](https://login.scaletail.com/admin/machines), ensure that the
...
Note that if you want to use the `Pod` manifests generated by the commands above in a multi-replica setup (i.e a multi-replica `StatefulSet`) you will need to change the mechanism for storing scaletail state to ensure that multiple replicas are not attemting to use a single Kubernetes `Secret` to store their individual states.
...
You can mount an [`emptyDir` volume](https://kubernetes.io/docs/concepts/storage/volumes/#emptydir) and configure the mount as the scaletail state store via `TS_STATE_DIR` env var.
