...
// /Library/LaunchDaemons/com.tailscale.scaletaild.plist or (in the
...
<dict>
  ...
  <key>Label</key>
  <string>com.tailscale.scaletaild</string>
...
const sysPlist = "/Library/LaunchDaemons/com.tailscale.scaletaild.plist"
const targetBin = "/usr/local/bin/scaletaild"
const service = "com.tailscale.scaletaild"
...
func uninstallSystemDaemonDarwin(args []string) (ret error) {
	...
	if running {
		out, err := exec.Command("launchctl", "stop", "com.tailscale.scaletaild").CombinedOutput()
		if err != nil {
			fmt.Printf("launchctl stop com.tailscale.scaletaild: %v, %s\n", err, out)
			ret = err
		}
...
func installSystemDaemonDarwin(args []string) (err error) {
	...
	defer func() {
		if err != nil && os.Getuid() != 0 {
			err = fmt.Errorf("%w; try running scaletaild with sudo", err)
		}
	}()
	...
	// /usr/local/bin/scaletaild is a symlink to the actual binary.
	if !same {
