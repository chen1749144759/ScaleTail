...
// The scaletaild program is the Tailscale client daemon. It's configured
// and controlled via the scaletail CLI program.
//
...
func main() {
	...
	flag.StringVar(&args.socketpath, "socket", paths.DefaultScaleTaildSocket(), "path of the service unix socket")
...
store state on filesystem.`)
	...
	if runtime.GOOS == "plan9" && os.Getenv("_NETSHELL_CHILD_") != "" {
		os.Args = []string{"scaletaild", "be-child", "plan9-netshell"}
	}
	...
	if flag.NArg() > 0 {
		...
		if runtime.GOOS != "windows" || (flag.Arg(0) != "/subproc" && flag.Arg(0) != "/firewall") {
			log.Fatalf("scaletaild does not take non-flag arguments: %q", flag.Args())
		}
	}
	...
	if runtime.GOOS == "darwin" && os.Getuid() != 0 && !strings.Contains(args.tunname, "userspace-networking") && !args.cleanUp {
		log.SetFlags(0)
		log.Fatalf("scaletaild requires root; use sudo scaletaild (or use --tun=userspace-networking)")
	}
	...
	if args.statepath == "" && args.statedir == "" {
		if paths.MakeAutomaticStateDir() {
			d := paths.DefaultScaleTaildStateDir()
			if d != "" {
				args.statedir = d
			...
			args.statepath = paths.DefaultScaleTaildStateFile()
		}
	}
...
func trySynologyMigration(p string) error {
	...
	const oldPath = "/var/packages/ScaleTail/etc/scaletaild.state"
...
func statePathOrDefault() string {
	...
	if path == "" && args.statedir != "" {
		path = filepath.Join(args.statedir, "scaletaild.state")
	}
...
type serverOptions struct {
	...
	// directory (usually "/var/lib/scaletail" on Linux) that
	// contains the "scaletaild.state" file, the "certs" directory
...
func run() (err error) {
	...
	// such as when a system was rebooted without shutting down, or scaletaild
...
func startIPNServer(ctx context.Context, logf logger.Logf, logID logid.PublicID, sys *tsd.System) error {
	...
	// scaletaild. The default action is to terminate the process, we
	...
	go func() {
		...
		for {
			select {
			case s := <-interrupt:
				logf("scaletaild got signal %v; shutting down", s)
				cancel()
				return
			case <-wgEngineClosed:
...
func handleSubnetsInNetstack() bool {
	...
	case "windows", "darwin", "freebsd", "openbsd", "solaris", "illumos":
		// Enable on Windows and scaletaild-on-macOS (this doesn't
...
func tryEngine(logf logger.Logf, sys *tsd.System, name string) (onlyNetstack bool, err error) {
	...
	if onlyNetstack {
		if runtime.GOOS == "linux" && distro.Get() == distro.Synology {
			...
			// warnings in 'scaletail status' about DNS base
...
// macOS. This helps us clean up straggler scaletaild processes when the parent
...
// applyIntegrationTestEnvKnob applies the scaletaild.env=... environment
// variables specified on the Linux kernel command line, if the VM is being
// run in NATLab integration tests.
//
// They're specified as: scaletaild.env=FOO=bar scaletaild.env=BAR=baz
func applyIntegrationTestEnvKnob() {
	...
	for _, s := range strings.Fields(string(cmdLine)) {
		suf, ok := strings.CutPrefix(s, "scaletaild.env=")
		if !ok {
			continue
		}
...
func canUseHardwareAttestation() error {
	if _, err := key.NewEmptyHardwareAttestationKey(); err == key.ErrUnsupported {
		return errors.New("--hardware-attestation is not supported on this platform or in this build of scaletaild")
	}
