...
//
// TODO: check if ScaleTail service is already running, and fail early
//       like tswin does.
//
...
func init() {
	if isWindowsService() {
		...
		// Otherwise, if scaletaild starts due to a GPSI policy installing Tailscale, it may deadlock
		// while waiting for the write counterpart of the GP lock to be released by Group Policy,
		// which is itself waiting for the installation to complete and scaletaild to start.
		// See tailscale/tailscale#14416 for more information.
		permitPolicyLocks = gp.RestrictPolicyLocks()
	}
}

const serviceName = "ScaleTail"
...
// one that you see in the Windows Event Viewer). scaletaild may optionally
// generate diagnostic messages in the same event timeline as the Windows
// Service Control Manager to assist with diagnosing issues with scaletaild's
	...
	for {
		...
		case cmd := <-r:
			...
			case cmdUninstallWinTun:
				syslogf("Stopping scaletaild child process and uninstalling WinTun")
				// At this point, doneCh is the channel which will be closed when the
				// scaletaild subprocess exits. We save that to childDoneCh.
				childDoneCh := doneCh
...
func beFirewallKillswitch() bool {
	...
	log.SetFlags(0)
	log.Printf("killswitch subprocess starting, scaletail GUID is %s", os.Args[2])
	...
	// Note(maisem): when local lan access toggled, scaletaild needs to
