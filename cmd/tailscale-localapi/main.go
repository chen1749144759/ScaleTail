...
// tailscale-localapi is a small helper used by the Windows Electron client.
// It talks to tailscaled through the official Go LocalAPI client so Windows
...
func main() {
	if len(os.Args) < 2 {
		fatalf("usage: tailscale-localapi request|watch [flags]")
	}
