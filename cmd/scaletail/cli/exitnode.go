...
func exitNodeCmd() *ffcli.Command {
	return &ffcli.Command{
		...
		Subcommands: append([]*ffcli.Command{
			...
			{
				Name:       "suggest",
				ShortUsage: "scaletail exit-node suggest",
			...
			(func() []*ffcli.Command {
				...
				return []*ffcli.Command{
					{
						Name:       "connect",
						ShortUsage: "scaletail exit-node connect",
						ShortHelp:  "Connect to most recently used exit node",
						Exec:       exitNodeSetUse(true),
					},
					{
						Name:       "disconnect",
						ShortUsage: "scaletail exit-node disconnect",
...
func runExitNodeList(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return errors.New("unexpected non-flag arguments to 'scaletail exit-node list'")
	}
	...
	for _, country := range filteredPeers.Countries {
		for _, city := range country.Cities {
			for _, peer := range city.Peers {
				fmt.Fprintf(w, "\n %s\t%s\t%s\t%s\t%s\t", peer.ScaleTailIPs[0], strings.Trim(peer.DNSName, "."), cmp.Or(country.Name, "-"), cmp.Or(city.Name, "-"), peerStatus(peer))
			}
		}
	}
	fmt.Fprintln(w)
	fmt.Fprintln(w)
	fmt.Fprintln(w, "# To view the complete list of exit nodes for a country, use `scaletail exit-node list --filter=` followed by the country name.")
	fmt.Fprintln(w, "# To use an exit node, use `scaletail set --exit-node=` followed by the IP or hostname.")
	if hasAnyExitNodeSuggestions(peers) {
		fmt.Fprintln(w, "# To have Tailscale suggest an exit node, use `scaletail exit-node suggest`.")
	}
	return nil
}
