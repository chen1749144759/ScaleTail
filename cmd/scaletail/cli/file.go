...
func getFileCmd() *ffcli.Command {
	return &ffcli.Command{
		Name:       "file",
		ShortUsage: "scaletail file <cp|get> ...",
...
var fileCpCmd = &ffcli.Command{
	Name:       "cp",
	ShortUsage: "scaletail file cp <files...> <target>:",
...
func runCp(ctx context.Context, args []string) error {
	...
	if len(args) < 2 {
		return errors.New("usage: scaletail file cp <files...> <target>:")
	}
	files, target := args[:len(args)-1], args[len(args)-1]
	target, ok := strings.CutSuffix(target, ":")
	if !ok {
		return fmt.Errorf("final argument to 'scaletail file cp' must end in colon")
	}
	hadBrackets := false
	...
	// to the IPN bus. scaletaild's OutgoingFile.Sent is the bytes-pulled-toward-
	...
	// unix-socket conn to scaletaild, well before the peer has heard a thing.
...
func watchOutgoingFiles(ctx context.Context, peer tailcfg.StableNodeID, onUpdate func(name string, sent int64)) {
	...
	for {
		...
		for _, of := range n.OutgoingFiles {
			...
			// scaletaild keeps Finished entries in its OutgoingFiles map
...
func progressPrinter(ctx context.Context, name string, contentCount func() int64, contentLength int64, interval time.Duration) {
	...
	// scaletaild emits OutgoingFile.Sent updates at ~1 Hz, so most printer
...
func getTargetStableID(ctx context.Context, ipStr string) (id tailcfg.StableNodeID, isOffline bool, err error) {
	...
	if err != nil {
		// This likely means scaletaild is unreachable or returned an error on /localapi/v0/status.
...
peerLoop:
	for _, ps := range st.Peer {
		for _, pip := range ps.ScaleTailIPs {
			if pip == ip {
				foundPeer = ps
				break peerLoop
			}
...
var fileGetCmd = &ffcli.Command{
	Name:       "get",
	ShortUsage: "scaletail file get [--wait] [--verbose] [--conflict=(skip|overwrite|rename)] <target-directory>",
...
func runFileGet(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail file get <target-directory>")
	}
	log.SetFlags(0)
	...
	if getArgs.loop {
		for {
			...
			if len(errs) > 0 {
				...
				//    scaletail file get --conflict=skip ~/Downloads
