...
var pingCmd = &ffcli.Command{
	Name:       "ping",
	ShortUsage: "scaletail ping <hostname-or-IP>",
	ShortHelp:  "Ping a host at the Tailscale layer, see how it routed",
	LongHelp: strings.TrimSpace(`

The 'scaletail ping' command pings a peer node from the Tailscale layer
...
If 'scaletail ping' works but a normal ping does not, that means one
side's operating system firewall is blocking packets; 'scaletail ping'
does not inject packets into either side's TUN devices.

By default, 'scaletail ping' stops after 10 pings or once a direct
...
func runPing(ctx context.Context, args []string) error {
	...
	if len(args) != 1 || args[0] == "" {
		return errors.New("usage: scaletail ping <hostname-or-IP>")
	}
	var ip string
...
func tailscaleIPFromArg(ctx context.Context, hostOrIP string) (ip string, self bool, err error) {
	...
	for _, ps := range st.Peer {
		if match(ps) {
			...
			return ps.ScaleTailIPs[0].String(), false, nil
		}
	}
	if match(st.Self) && len(st.Self.ScaleTailIPs) > 0 {
		return st.Self.ScaleTailIPs[0].String(), true, nil
	}
