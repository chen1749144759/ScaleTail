...
// Package cli contains the cmd/scaletail CLI code in a package that can be included
...
var localClient = local.Client{
	Socket: paths.DefaultScaleTaildSocket(),
}
...
func Run(args []string) (err error) {
	if runtime.GOOS == "linux" && os.Getenv("GOKRAZY_FIRST_START") == "1" && distro.Get() == distro.Gokrazy && os.Getppid() == 1 && len(args) == 0 {
		...
		// Don't run the scaletail CLI and spam logs with usage; just exit.
	...
	local.SetVersionMismatchHandler(func(clientVer, serverVer string) {
		warnOnce.Do(func() {
			fmt.Fprintf(Stderr, "Warning: client version %q != scaletaild server version %q\n", clientVer, serverVer)
		})
	})
	...
	if local.IsAccessDeniedError(err) && os.Getuid() != 0 && runtime.GOOS != "windows" {
		return fmt.Errorf("%v\n\nUse 'sudo scaletail %s'.\nTo not require root, use 'sudo scaletail set --operator=$USER' once.", err, strings.Join(args, " "))
	}
	if errors.Is(err, flag.ErrHelp) {
		return nil
	}
...
func newRootCmd(tb ...testenv.TB) *ffcli.Command {
	rootfs := newFlagSet("scaletail")
	rootfs.Func("socket", "path to scaletaild socket", func(s string) error {
	...
	rootCmd = &ffcli.Command{
		Name:       "scaletail",
		ShortUsage: "scaletail [flags] <subcommand> [command flags]",
		ShortHelp:  "The easiest, most secure way to use WireGuard.",
		LongHelp: strings.TrimSpace(`
For help on subcommands, add --help after: "scaletail status --help".
...
`),
		...
		Exec: func(ctx context.Context, args []string) error {
			...
			if len(args) > 0 {
				return fmt.Errorf("scaletail: unknown subcommand: %s", args[0])
			}
			return flag.ErrHelp
		},
	}
...
// fullCmd returns the full "scaletail ... cmd" invocation for a subcommand.
...
var hookFixScaleTaildConnectError feature.Hook[func(error) error] // for cliconndiag

func fixScaleTaildConnectError(origErr error) error {
	if f, ok := hookFixScaleTaildConnectError.GetOk(); ok {
		return f(origErr)
	}
	return origErr
}
