...
var idTokenCmd = &ffcli.Command{
	Name:       "id-token",
	ShortUsage: "scaletail id-token <aud>",
...
func runIDToken(ctx context.Context, args []string) error {
	if !envknob.UseWIPCode() {
		return errors.New("scaletail id-token: works-in-progress require TAILSCALE_USE_WIP_CODE=1 envvar")
	}
	if len(args) != 1 {
		return errors.New("usage: scaletail id-token <aud>")
	}
