...
var infoMap = map[serveMode]commandInfo{
	serve: {
		...
		LongHelp: strings.Join([]string{
			...
			"To share a local server on the internet, use `scaletail funnel`\n\n",
		}, "\n"),
	},
	funnel: {
		Name:      "funnel",
		...
		LongHelp: strings.Join([]string{
			...
			"To share only within your tailnet, use `scaletail serve`\n\n",
		}, "\n"),
	},
}
...
var errHelpFunc = func(m serveMode) error {
	return fmt.Errorf("try `scaletail %s --help` for usage info", infoMap[m].Name)
}
...
func newServeV2Command(e *serveEnv, subcmd serveMode) *ffcli.Command {
	...
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			fmt.Sprintf("scaletail %s <target>", info.Name),
			fmt.Sprintf("scaletail %s status [--json]", info.Name),
			fmt.Sprintf("scaletail %s reset", info.Name),
		}, "\n"),
		...
		Subcommands: func() []*ffcli.Command {
			subcmds := []*ffcli.Command{
				{
					Name:       "status",
					ShortUsage: "scaletail " + info.Name + " status [--json]",
				...
				{
					Name:       "reset",
					ShortUsage: "scaletail " + info.Name + " reset",
			...
			if subcmd == serve {
				subcmds = append(subcmds, []*ffcli.Command{
					{
						Name:       "drain",
						ShortUsage: fmt.Sprintf("scaletail %s drain <service>", info.Name),
					...
					{
						Name:       "clear",
						ShortUsage: fmt.Sprintf("scaletail %s clear <service>", info.Name),
					...
					{
						Name:       "advertise",
						ShortUsage: fmt.Sprintf("scaletail %s advertise <service>", info.Name),
						...
						LongHelp: "Advertise this node as a service proxy to the tailnet. This command is used\n" +
							...
							"`scaletail serve drain <service>`). This is not needed if you are using `scaletail serve` to initialize a service.",
						Exec: e.runServeAdvertise,
					},
					{
						Name:       "get-config",
						ShortUsage: fmt.Sprintf("scaletail %s get-config <file> [--service=<service>] [--all]", info.Name),
					...
					{
						Name:       "set-config",
						ShortUsage: fmt.Sprintf("scaletail %s set-config <file> [--service=<service>] [--all]", info.Name),
...
// runServeCombined is the entry point for the "scaletail {serve,funnel}" commands.
...
var (
	...
	msgDisableProxy                = "To disable the proxy, run: scaletail %s --%s=%d off"
...
func (e *serveEnv) shouldWarnRemoteDestCompatibility(ctx context.Context, target string) error {
	...
	// and scaletail bypass mark is not working. If any of these conditions are true, and the dest is
	...
	if runtime.GOOS == "linux" {
		...
		if !SOMarkInUse {
			return fmt.Errorf(msgWarnRemoteDestCompatibility, "the Linux scaletaild without SO_MARK")
		}
	}
