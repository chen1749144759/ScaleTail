...
var setCmd = &ffcli.Command{
	Name:       "set",
	ShortUsage: "scaletail set [flags]",
	ShortHelp:  "Change specified preferences",
	LongHelp: `"scaletail set" allows changing specific preferences.

Unlike "scaletail up", this command does not require the complete set of desired settings.
...
func newSetFlagSet(goos string, setArgs *setArgsT) *flag.FlagSet {
	...
	if safesocket.GOOSUsesPeerCreds(goos) {
		setf.StringVar(&setArgs.opUser, "operator", "", "Unix username to allow to operate on scaletaild without sudo")
	}
	switch goos {
	case "linux":
...
func runSet(ctx context.Context, args []string) (retErr error) {
	...
	if setArgs.runWebClient && len(st.ScaleTailIPs) > 0 {
		printf("\nWeb interface now running at %s:%d\n", st.ScaleTailIPs[0], tsconst.WebListenPort)
	}
...
// current value, the flags passed to "scaletail set".
