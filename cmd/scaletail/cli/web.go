...
func webCmd() *ffcli.Command {
	return &ffcli.Command{
		Name:       "web",
		ShortUsage: "scaletail web [flags]",
		ShortHelp:  "Run a web server for controlling Tailscale",

		LongHelp: strings.TrimSpace(`
"scaletail web" runs a webserver for controlling the Tailscale daemon.
...
func tlsConfigFromEnvironment() *tls.Config {
	...
	// TLS_CRT_PEM and TLS_KEY_PEM from PHP code before starting scaletail web.
...
func runWeb(ctx context.Context, args []string) error {
	...
	if err == nil && st.Self != nil && len(st.Self.ScaleTailIPs) > 0 {
		selfIP = st.Self.ScaleTailIPs[0]
	}
	...
	if !existingWebClient && !webArgs.readonly {
		// Also start full client in scaletaild.
		log.Printf("starting scaletaild web client at http://%s\n", netip.AddrPortFrom(selfIP, tsconst.WebListenPort))
		if err := setRunWebClient(ctx, true); err != nil {
			return fmt.Errorf("starting web client in scaletaild: %w", err)
		}
	...
	go func() {
		...
		case <-ctx.Done():
			...
			if !webArgs.cgi && startedManagementClient {
				log.Println("stopping scaletaild web client")
				// When not in cgi mode, shut down the scaletaild
				// web client on cli termination if we started it.
				if err := setRunWebClient(context.Background(), false); err != nil {
					log.Printf("stopping scaletaild web client: %v", err)
				}
			}
		}
		os.Exit(0)
