...
func debugCmd() *ffcli.Command {
	return &ffcli.Command{
		Name:       "debug",
		Exec:       runDebug,
		ShortUsage: "scaletail debug <debug-flags | subcommand>",
		ShortHelp:  "Debug commands",
		LongHelp:   hidden + `"scaletail debug" contains misc debug facilities; it is not a stable interface.`,
		...
		Subcommands: nonNilCmds([]*ffcli.Command{
			{
				Name:       "derp-map",
				ShortUsage: "scaletail debug derp-map",
				Exec:       runDERPMap,
				ShortHelp:  "Print DERP map",
			},
			{
				Name:       "component-logs",
				ShortUsage: "scaletail debug component-logs [" + strings.Join(ipn.DebuggableComponents, "|") + "]",
			...
			},
			{
				...
				ShortUsage: "scaletail debug daemon-goroutines",
				Exec:       runDaemonGoroutines,
				ShortHelp:  "Print scaletaild's goroutines",
			},
			{
				Name:       "daemon-logs",
				ShortUsage: "scaletail debug daemon-logs",
				Exec:       runDaemonLogs,
				ShortHelp:  "Watch scaletaild's server logs",
				FlagSet: (func() *flag.FlagSet {
			...
			},
			{
				...
				ShortUsage: "scaletail debug daemon-bus-events",
				Exec:       runDaemonBusEvents,
				ShortHelp:  "Watch events on the scaletaild bus",
			},
			{
				Name:       "daemon-bus-graph",
				ShortUsage: "scaletail debug daemon-bus-graph",
				Exec:       runDaemonBusGraph,
				ShortHelp:  "Print graph for the scaletaild bus",
				FlagSet: (func() *flag.FlagSet {
			...
			},
			{
				...
				ShortUsage: "scaletail debug daemon-bus-queues",
				Exec:       runDaemonBusQueues,
				ShortHelp:  "Print event bus queue depths per client",
			},
			{
				Name:       "metrics",
				ShortUsage: "scaletail debug metrics",
				Exec:       runDaemonMetrics,
				ShortHelp:  "Print scaletaild's metrics",
				FlagSet: (func() *flag.FlagSet {
			...
			{
				Name:       "env",
				ShortUsage: "scaletail debug env",
				Exec:       runEnv,
				ShortHelp:  "Print cmd/scaletail environment",
			},
			{
				Name:       "stat",
				ShortUsage: "scaletail debug stat <files...>",
				Exec:       runStat,
				ShortHelp:  "Stat a file",
			},
			{
				Name:       "hostinfo",
				ShortUsage: "scaletail debug hostinfo",
				Exec:       runHostinfo,
				ShortHelp:  "Print hostinfo",
			},
			{
				Name:       "local-creds",
				ShortUsage: "scaletail debug local-creds",
				Exec:       runLocalCreds,
				ShortHelp:  "Print how to access Tailscale LocalAPI",
			},
			{
				Name:       "localapi",
				ShortUsage: "scaletail debug localapi [<method>] <path> [<body| \"-\">]",
				Exec:       runLocalAPI,
			...
			},
			{
				...
				Name:       "reload-config",
				ShortUsage: "scaletail debug reload-config",
				Exec:       reloadConfig,
				ShortHelp:  "Reload config",
			},
			{
				Name:       "control-knobs",
				ShortUsage: "scaletail debug control-knobs",
				Exec:       debugControlKnobs,
				ShortHelp:  "See current control knobs",
			},
			{
				Name:       "prefs",
				ShortUsage: "scaletail debug prefs",
				Exec:       runPrefs,
			...
			{
				Name:       "watch-ipn",
				ShortUsage: "scaletail debug watch-ipn",
				Exec:       runWatchIPN,
			...
			{
				Name:       "netmap",
				ShortUsage: "scaletail debug netmap",
				Exec:       runNetmap,
			...
			{
				Name: "via",
				ShortUsage: "scaletail debug via <site-id> <v4-cidr>\n" +
					"scaletail debug via <v6-route>",
				Exec:      runVia,
				ShortHelp: "Convert between site-specific IPv4 CIDRs and IPv6 'via' routes",
			},
			{
				Name:       "ts2021",
				ShortUsage: "scaletail debug ts2021",
				Exec:       runTS2021,
			...
			{
				Name:       "set-expire",
				ShortUsage: "scaletail debug set-expire --in=1m",
				Exec:       runSetExpire,
			...
			{
				Name:       "dev-store-set",
				ShortUsage: "scaletail debug dev-store-set",
				Exec:       runDevStoreSet,
			...
			{
				Name:       "derp",
				ShortUsage: "scaletail debug derp",
				Exec:       runDebugDERP,
			...
			ccall(debugPortmapCmd),
			{
				...
				ShortUsage: "scaletail debug peer-endpoint-changes <hostname-or-IP>",
				Exec:       runPeerEndpointChanges,
				ShortHelp:  "Print debug information about a peer's endpoint changes",
			},
			{
				Name:       "dial-types",
				ShortUsage: "scaletail debug dial-types <hostname-or-IP> <port>",
			...
			{
				Name:       "resolve",
				ShortUsage: "scaletail debug resolve <hostname>",
			...
			{
				Name:       "statedir",
				ShortUsage: "scaletail debug statedir",
...
func runDebug(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("scaletail debug: unknown subcommand: %s", args[0])
	}
	var usedFlag bool
	...
	return errors.New("scaletail debug: subcommand or flag required")
}
...
func runLocalCreds(ctx context.Context, args []string) error {
	...
	printf("curl --unix-socket %s http://local-scaletaild.sock/localapi/v0/status\n", paths.DefaultScaleTaildSocket())
	return nil
}
...
func runLocalAPI(ctx context.Context, args []string) error {
	...
	req, err := http.NewRequest(method, "http://local-scaletaild.sock"+path, body)
	if err != nil {
		return err
	}
...
func runDebugComponentLogs(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail debug component-logs [" + strings.Join(ipn.DebuggableComponents, "|") + "]")
	}
	component := args[0]
...
func runDevStoreSet(ctx context.Context, args []string) error {
	if len(args) != 2 {
		return errors.New("usage: scaletail debug dev-store-set --danger <key> <value>")
	}
...
func runDebugDERP(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail debug derp <region>")
	}
...
func runSetExpire(ctx context.Context, args []string) error {
	if len(args) != 0 || setExpireArgs.in == 0 {
		return errors.New("usage: scaletail debug set-expire --in=<duration>")
	}
...
func runPeerEndpointChanges(ctx context.Context, args []string) error {
	...
	if len(args) != 1 || args[0] == "" {
		return errors.New("usage: scaletail debug peer-endpoint-changes <hostname-or-IP>")
	}
	var ip string
	...
	req, err := http.NewRequestWithContext(ctx, "GET", "http://local-scaletaild.sock/localapi/v0/debug-peer-endpoint-changes?ip="+ip, nil)
	if err != nil {
		return err
	}
...
func runDebugDialTypes(ctx context.Context, args []string) error {
	...
	if len(args) != 2 || args[0] == "" || args[1] == "" {
		return errors.New("usage: scaletail debug dial-types <hostname-or-IP> <port>")
	}
	...
	req, err := http.NewRequestWithContext(ctx, "POST", "http://local-scaletaild.sock/localapi/v0/debug-dial-types?"+qparams.Encode(), nil)
	if err != nil {
		return err
	}
...
func runDebugResolve(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail debug resolve <hostname>")
	}
