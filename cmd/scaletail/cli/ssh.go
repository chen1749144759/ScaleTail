...
var sshCmd = &ffcli.Command{
	Name:       "ssh",
	ShortUsage: "scaletail ssh [user@]<host> [args...]",
	ShortHelp:  "SSH to a Tailscale machine",
	LongHelp: strings.TrimSpace(`

The 'scaletail ssh' command is an optional wrapper around the system 'ssh'
...
The 'scaletail ssh' wrapper adds a few things:
...
* It works in userspace-networking mode, by supplying a ProxyCommand to the
  system 'ssh' command that connects via a pipe through scaletaild.
...
func runSSH(ctx context.Context, args []string) error {
	if runtime.GOOS == "darwin" && version.IsMacAppStore() && !envknob.UseWIPCode() {
		return errors.New("The 'scaletail ssh' subcommand is not available on macOS builds distributed through the App Store or TestFlight.\nInstall the Standalone variant of Tailscale (download it from https://pkgs.tailscale.com), or use the regular 'ssh' client instead.")
	}
	if len(args) == 0 {
		return errors.New("usage: scaletail ssh [user@]<host>")
	}
	...
	if runtime.GOOS != "darwin" {
		socketArg := ""
		if localClient.Socket != "" && localClient.Socket != paths.DefaultScaleTaildSocket() {
			socketArg = fmt.Sprintf("--socket=%q", localClient.Socket)
		}

		argv = append(argv,
			"-o", fmt.Sprintf("ProxyCommand %q %s nc %%h %%p",
				// os.Executable() would return the real running binary but in case scaletail is built with the ts_include_cli tag,
	...
	// be compatible, but only a subset. The "scaletail ssh"
...
func genKnownHosts(st *ipnstate.Status) []byte {
	...
	for _, k := range st.Peers() {
		...
		for _, hk := range ps.SSH_HostKeys {
			...
			fmt.Fprintf(&buf, "%s %s\n", ps.DNSName, hostKey)
			for _, ip := range ps.ScaleTailIPs {
				fmt.Fprintf(&buf, "%s %s\n", ip.String(), hostKey)
			}
		}
	}
...
func peerStatusFromArg(st *ipnstate.Status, arg string) (*ipnstate.PeerStatus, bool) {
	...
	for _, ps := range st.Peer {
		if argIP.IsValid() {
			if slices.Contains(ps.ScaleTailIPs, argIP) {
				return ps, true
			}
			continue
		}
...
func ipFromPeerStatus(ps *ipnstate.PeerStatus) (string, bool) {
	...
	return ps.ScaleTailIPs[0].String(), true
}
