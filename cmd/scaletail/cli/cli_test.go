...
// cutWord("scaletail debug scale 123", "scale") returns (" 123", true).
...
func TestCutWord(t *testing.T) {
	tests := []struct {
		...
		{"scaletail debug", "debug", "", true},
		{"scaletail debug", "bug", "", false},
		{"scaletail debug", "tail", "", false},
		{"scaletail debug scaley scale 123", "scale", " 123", true},
	}
...
func TestPrefsFromUpArgs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			st: &ipnstate.Status{
				ScaleTailIPs: []netip.Addr{netip.MustParseAddr("100.105.106.107")},
			},
...
func TestPrefFlagMapping(t *testing.T) {
	...
	for field := range prefType.Fields() {
		...
		case "NetfilterKind":
			...
			continue
		case "DriveShares":
			// Handled by the scaletail share subcommand, we don't want a CLI
			// flag for this.
			continue
		case "AdvertiseServices":
			// Handled by the scaletail serve subcommand, we don't want a
			// CLI flag for this.
			continue
		case "InternalExitNodePrior":
			...
			continue
		case "AutoExitNode":
			// Handled by scaletail {set,up} --exit-node=auto:any.
			continue
		}
...
func TestUpdatePrefs(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantSimpleUp: false,
			wantErrSubtr: "scaletail up --accept-dns=false",
		},
		{
			name:  "just_edit",
			flags: []string{},
		...
		},
		{
			...
			wantJustEditMP: &ipn.MaskedPrefs{
				WantRunningSet:  true, // enabled by default for scaletail up
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.sshOverTailscale {
				...
				// The test is being executed over a "real" scaletail SSH
				// session, but sshOverTailscale is unset. Make the test appear
				// as if it's not over scaletail SSH.
...
func TestDocs(t *testing.T) {
	...
	check := func(t *testing.T, c *ffcli.Command) {
		...
		switch name {
		case "TestDocs/scaletail/completion/bash",
			"TestDocs/scaletail/completion/zsh":
