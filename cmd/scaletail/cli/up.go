...
func newUpFlagSet(goos string, upArgs *upArgsT, cmd string) *flag.FlagSet {
	...
	if safesocket.GOOSUsesPeerCreds(goos) {
		upf.StringVar(&upArgs.opUser, "operator", "", "Unix username to allow to operate on scaletaild without sudo")
	}
	switch goos {
	case "linux":
	...
	upf.DurationVar(&upArgs.timeout, "timeout", 0, "maximum amount of time to wait for scaletaild to enter a Running state; default (0s) blocks forever")
...
//
// It returns simpleUp if we're running a simple "scaletail up" to
...
func runUp(ctx context.Context, cmd string, args []string, upArgs upArgsT) (retErr error) {
	...
	// Special case: bare "scaletail up" means to just start
...
// about during "scaletail up". Many of the health checks are noisy or confusing
...
const accidentalUpPrefix = "Error: changing settings via 'scaletail up' requires mentioning all\n" +
...
// people running "scaletail up" with a subset of the flags they
...
func checkForAccidentalSettingReverts(newPrefs, curPrefs *ipn.Prefs, env upCheckEnv) (simpleUp bool, err error) {
	...
	if len(flagIsSet) == 0 &&
		...
		// A bare "scaletail up" is a special case to just
...
func exitNodeIP(p *ipn.Prefs, st *ipnstate.Status) (ip netip.Addr) {
	...
	for _, p := range st.Peer {
		if p.ID == id {
			if len(p.ScaleTailIPs) > 0 {
				return p.ScaleTailIPs[0]
			}
			break
		}
	}
