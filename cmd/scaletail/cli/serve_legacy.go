...
func newServeLegacyCommand(e *serveEnv) *ffcli.Command {
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			"scaletail serve http:<port> <mount-point> <source> [off]",
			"scaletail serve https:<port> <mount-point> <source> [off]",
			"scaletail serve tcp:<port> tcp://localhost:<local-port> [off]",
			"scaletail serve tls-terminated-tcp:<port> tcp://localhost:<local-port> [off]",
			"scaletail serve status [--json]",
			"scaletail serve reset",
		}, "\n"),
		LongHelp: strings.TrimSpace(`
*** BETA; all of this is subject to change ***

The 'scaletail serve' set of commands allows you to serve
...
EXAMPLES
  - To proxy requests to a web server at 127.0.0.1:3000:
    $ scaletail serve https:443 / http://127.0.0.1:3000

    Or, using the default port (443):
    $ scaletail serve https / http://127.0.0.1:3000

  - To serve a single file or a directory of files:
    $ scaletail serve https / /home/alice/blog/index.html
    $ scaletail serve https /images/ /home/alice/blog/images

  - To serve simple static text:
    $ scaletail serve https:8080 / text:"Hello, world!"

  - To serve over HTTP (tailnet only):
    $ scaletail serve http:80 / http://127.0.0.1:3000

    Or, using the default port (80):
    $ scaletail serve http / http://127.0.0.1:3000
  ...
  - To forward incoming TCP connections on port 2222 to a local TCP server on
    ...
    $ scaletail serve tcp:2222 tcp://localhost:22

  - To accept TCP TLS connections (terminated within scaletaild) proxied to a
    local plaintext server on port 80:
    $ scaletail serve tls-terminated-tcp:443 tcp://localhost:80
`),
		Exec: e.runServe,
...
var errHelp = errors.New("try `scaletail serve --help` for usage info")
...
//
// Returns error if unable to reach scaletaild or if self node is nil.
//
...
func (e *serveEnv) getLocalClientStatusWithoutPeers(ctx context.Context) (*ipnstate.Status, error) {
	...
	if err != nil {
		return nil, fixScaleTaildConnectError(err)
	}
...
//
// Examples:
// - scaletail serve http / http://localhost:3000
// - scaletail serve https / http://localhost:3000
// - scaletail serve https /images/ /var/www/images/
// - scaletail serve https:10000 /motd.txt text:"Hello, world!"
// - scaletail serve tcp:2222 tcp://localhost:22
// - scaletail serve tls-terminated-tcp:443 tcp://localhost:80
...
// handleWebServe handles the "scaletail serve (http/https):..." subcommand. It
// configures the serve config to forward HTTPS connections to the given source.
//
// Examples:
//   - scaletail serve http / http://localhost:3000
//   - scaletail serve https / http://localhost:3000
//   - scaletail serve https:8443 /files/ /home/alice/shared-files/
//   - scaletail serve https:10000 /motd.txt text:"Hello, world!"
...
// handleTCPServe handles the "scaletail serve tls-terminated-tcp:..." subcommand.
// It configures the serve config to forward TCP connections to the
// given source.
//
// Examples:
//   - scaletail serve tcp:2222 tcp://localhost:22
//   - scaletail serve tls-terminated-tcp:8443 tcp://localhost:8080
...
func (e *serveEnv) enableFeatureInteractive(ctx context.Context, feature string, caps ...tailcfg.NodeCapability) (err error) {
	...
	if err != nil {
		...
		log.Fatalf("lost connection to scaletaild: %v", err)
