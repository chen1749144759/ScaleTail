...
var dnsQueryCmd = &ffcli.Command{
	Name:       "query",
	ShortUsage: "scaletail dns query [--json] <name> [type]",
	Exec:       runDNSQuery,
	ShortHelp:  "Perform a DNS query",
	LongHelp: strings.TrimSpace(`
The 'scaletail dns query' subcommand performs a DNS query for the specified name
...
func runDNSQuery(ctx context.Context, args []string) error {
	...
	if len(args) > 1 {
		...
		if len(flags) > 0 {
			return fmt.Errorf("unexpected flags after query name: %s; see 'scaletail dns query --help'", strings.Join(flags, ", "))
		}
		if len(args) > 2 {
