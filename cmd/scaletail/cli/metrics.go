...
The 'scaletail metrics' command shows Tailscale user-facing metrics (as opposed
to internal metrics printed by 'scaletail debug metrics').

For more information about Tailscale metrics, refer to
https://scaletail.com/s/client-metrics

`),
	ShortUsage: "scaletail metrics <subcommand> [flags]",
	UsageFunc:  usageFuncNoDefaultValues,
	Exec:       runMetricsNoSubcommand,
	Subcommands: []*ffcli.Command{
		{
			Name:       "print",
			ShortUsage: "scaletail metrics print",
			Exec:       runMetricsPrint,
			ShortHelp:  "Print current metric values in Prometheus text format",
		},
		{
			Name:       "write",
			ShortUsage: "scaletail metrics write <path>",
			Exec:       runMetricsWrite,
			ShortHelp:  "Write metric values to a file",
			LongHelp: strings.TrimSpace(`

The 'scaletail metrics write' command writes metric values to a text file provided as its
...
can regularly run 'scaletail metrics write /var/lib/prometheus/node-exporter/scaletaild.prom'
...
func runMetricsNoSubcommand(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("scaletail metrics: unknown subcommand: %s", args[0])
	}
...
func runMetricsWrite(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail metrics write <path>")
	}
	path := args[0]
	out, err := localClient.UserMetrics(ctx)
	if err != nil {
