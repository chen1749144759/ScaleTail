...
// scaletaild distribution. On GUI builds, the Swift code in the macOS client handles these commands by not passing the
...
func sysExtCmd() *ffcli.Command {
	return &ffcli.Command{
		Name:       "sysext",
		ShortUsage: "scaletail configure sysext [activate|deactivate|status]",
		...
		Subcommands: []*ffcli.Command{
			{
				Name:       "activate",
				ShortUsage: "scaletail configure sysext activate",
				ShortHelp:  "Register the Tailscale system extension with macOS.",
				LongHelp:   "This command registers the Tailscale system extension with macOS. To run Tailscale, you'll also need to install the VPN configuration separately (run `scaletail configure vpn-config install`). After running this command, you need to approve the extension in System Settings > Login Items and Extensions > Network Extensions.",
				Exec:       requiresStandalone,
			},
			{
				Name:       "deactivate",
				ShortUsage: "scaletail configure sysext deactivate",
				ShortHelp:  "Deactivate the Tailscale system extension on macOS",
				LongHelp:   "This command deactivates the Tailscale system extension on macOS. To completely remove Tailscale, you'll also need to delete the VPN configuration separately (use `scaletail configure vpn-config uninstall`).",
				Exec:       requiresStandalone,
			},
			{
				Name:       "status",
				ShortUsage: "scaletail configure sysext status",
				ShortHelp:  "Print the enablement status of the Tailscale system extension",
				LongHelp:   "This command prints the enablement status of the Tailscale system extension. If the extension is not enabled, run `scaletail sysext activate` to enable it.",
				Exec:       requiresStandalone,
			},
		},
		Exec: requiresStandalone,
...
func vpnConfigCmd() *ffcli.Command {
	return &ffcli.Command{
		Name:       "mac-vpn",
		ShortUsage: "scaletail configure mac-vpn [install|uninstall]",
		...
		Subcommands: []*ffcli.Command{
			{
				Name:       "install",
				ShortUsage: "scaletail configure mac-vpn install",
				ShortHelp:  "Write the Tailscale VPN configuration to the macOS settings",
				LongHelp:   "This command writes the Tailscale VPN configuration to the macOS settings. This is the entry that appears in System Settings > VPN. If you are running the Standalone variant of the client, you'll also need to install the system extension separately (run `scaletail configure sysext activate`).",
				Exec:       requiresGUI,
			},
			{
				Name:       "uninstall",
				ShortUsage: "scaletail configure mac-vpn uninstall",
				ShortHelp:  "Delete the Tailscale VPN configuration from the macOS settings",
				LongHelp:   "This command removes the Tailscale VPN configuration from the macOS settings. This is the entry that appears in System Settings > VPN. If you are running the Standalone variant of the client, you'll also need to deactivate the system extension separately (run `scaletail configure sysext deactivate`).",
				Exec:       requiresGUI,
			},
		},
