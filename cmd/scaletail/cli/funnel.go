...
// Funnel allows you to publish a 'scaletail serve' server publicly, open to the
// entire internet.
...
func newFunnelCommand(e *serveEnv) *ffcli.Command {
	return &ffcli.Command{
		...
		ShortUsage: strings.Join([]string{
			"scaletail funnel <serve-port> {on|off}",
			"scaletail funnel status [--json]",
		}, "\n"),
		LongHelp: strings.Join([]string{
			"Funnel allows you to publish a 'scaletail serve'",
		...
		Subcommands: []*ffcli.Command{
			{
				...
				ShortUsage: "scaletail funnel status [--json]",
...
// runFunnel is the entry point for the "scaletail funnel" subcommand and
...
func printFunnelWarning(sc *ipn.ServeConfig) {
	...
	if warn {
		fmt.Fprintf(Stderr, "         run: `scaletail serve --help` to see how to configure handlers\n")
	}
}
