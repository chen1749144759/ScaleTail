...
var statusCmd = &ffcli.Command{
	Name:       "status",
	ShortUsage: "scaletail status [--active] [--web] [--json]",
	ShortHelp:  "Show state of scaletaild and its connections",
...
func runStatus(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return errors.New("unexpected non-flag arguments to 'scaletail status'")
	}
	...
	if locBasedExitNode {
		outln()
		printf("# To see the full list of exit nodes, including location-based exit nodes, run `scaletail exit-node list`  \n")
	}
	if len(st.Health) > 0 {
		outln()
		printHealth()
