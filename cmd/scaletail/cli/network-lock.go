...
var netlockCmd = &ffcli.Command{
	Name:       "lock",
	ShortUsage: "scaletail lock <subcommand> [arguments...]",
...
var nlInitCmd = &ffcli.Command{
	Name:       "init",
	ShortUsage: "scaletail lock init [--gen-disablement-for-support] --gen-disablements N <trusted-key>...",
	ShortHelp:  "Initialize tailnet lock",
	LongHelp: strings.TrimSpace(`

The 'scaletail lock init' command initializes tailnet lock for the
...
running 'scaletail lock' on that node, and copying the node's tailnet
lock key.

To disable tailnet lock, use the 'scaletail lock disable' command
...
var nlStatusCmd = &ffcli.Command{
	Name:       "status",
	ShortUsage: "scaletail lock status",
...
func runNetworkLockStatus(ctx context.Context, args []string) error {
	if len(args) > 0 {
		return fmt.Errorf("scaletail lock status: unexpected argument")
	}
	...
	if st.Enabled && len(st.FilteredPeers) > 0 {
		...
		for _, p := range st.FilteredPeers {
			...
			line.WriteString("\t")
			for i, addr := range p.ScaleTailIPs {
				line.WriteString(addr.String())
				if i < len(p.ScaleTailIPs)-1 {
					line.WriteString(",")
				}
			}
			line.WriteString("\t")
...
var nlAddCmd = &ffcli.Command{
	Name:       "add",
	ShortUsage: "scaletail lock add <public-key>...",
...
var nlRemoveCmd = &ffcli.Command{
	Name:       "remove",
	ShortUsage: "scaletail lock remove [--re-sign=false] <public-key>...",
...
func runNetworkLockRemove(ctx context.Context, args []string) error {
	...
	if len(st.TrustedKeys) == 1 {
		return errors.New("cannot remove the last trusted signing key; use 'scaletail lock disable' to disable tailnet lock instead, or add another signing key before removing one")
	}
...
func runNetworkLockSign(ctx context.Context, args []string) error {
	...
	if len(args) == 0 || len(args) > 2 {
		return errors.New("usage: scaletail lock sign <node-key> [<rotation-key>]")
	}
...
var nlDisableCmd = &ffcli.Command{
	Name:       "disable",
	ShortUsage: "scaletail lock disable <disablement-secret>",
	ShortHelp:  "Consume a disablement secret to shut down tailnet lock for the tailnet",
	LongHelp: strings.TrimSpace(`

The 'scaletail lock disable' command uses the specified disablement
...
func runNetworkLockDisable(ctx context.Context, args []string) error {
	...
	if len(secrets) != 1 {
		return errors.New("usage: scaletail lock disable <disablement-secret>")
	}
...
var nlLocalDisableCmd = &ffcli.Command{
	Name:       "local-disable",
	ShortUsage: "scaletail lock local-disable",
	ShortHelp:  "Disable tailnet lock for this node only",
	LongHelp: strings.TrimSpace(`

The 'scaletail lock local-disable' command disables tailnet lock for only
the current node.
...
var nlDisablementKDFCmd = &ffcli.Command{
	...
	ShortUsage: "scaletail lock disablement-kdf <hex-encoded-disablement-secret>",
...
func runNetworkLockDisablementKDF(ctx context.Context, args []string) error {
	if len(args) != 1 {
		return errors.New("usage: scaletail lock disablement-kdf <hex-encoded-disablement-secret>")
	}
...
var nlLogCmd = &ffcli.Command{
	Name:       "log",
	ShortUsage: "scaletail lock log [--limit N]",
...
var nlRevokeKeysCmd = &ffcli.Command{
	Name:       "revoke-keys",
	ShortUsage: "scaletail lock revoke-keys <tailnet-lock-key>...\n  revoke-keys [--cosign] [--finish] <recovery-blob>",
...
Revocation is a multi-step process that requires several signing nodes to ` + "`--cosign`" + ` the revocation. Use ` + "`scaletail lock remove`" + ` instead if the key has not been compromised.

1. To start, run ` + "`scaletail revoke-keys <tlpub-keys>`" + ` with the tailnet lock keys to revoke.
