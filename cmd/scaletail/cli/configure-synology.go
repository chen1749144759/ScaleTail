...
// configureHostCmd is the "scaletail configure-host" command which was once
// used to configure Synology devices, but is now a compatibility alias to
// "scaletail configure synology".
//
// It returns nil if the actual "scaletail configure synology" command is not
// available.
func configureHostCmd() *ffcli.Command {
	...
	return &ffcli.Command{
		...
		ShortUsage: "scaletail configure-host\n" + synologyConfigureCmd.ShortUsage,
...
func synologyConfigureCmd() *ffcli.Command {
	...
	return &ffcli.Command{
		Name:       "synology",
		Exec:       runConfigureSynology,
		ShortUsage: "scaletail configure synology",
...
create the /dev/net/tun device and give the scaletaild binary permission
to use it.

See: https://scaletail.com/s/synology-outbound
`),
		FlagSet: (func() *flag.FlagSet {
...
func runConfigureSynology(ctx context.Context, args []string) error {
	...
	const daemonBin = "/var/packages/Tailscale/target/bin/scaletaild"
	if _, err := os.Stat(daemonBin); err != nil {
		if os.IsNotExist(err) {
			return fmt.Errorf("scaletaild binary not found at %s. Is the Tailscale *.spk package installed?", daemonBin)
		}
		return err
	}
