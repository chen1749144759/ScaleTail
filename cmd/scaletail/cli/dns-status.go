...
var dnsStatusCmd = &ffcli.Command{
	Name:       "status",
	ShortUsage: "scaletail dns status [--all] [--json]",
	Exec:       runDNSStatus,
	ShortHelp:  "Print the current DNS status and configuration",
	LongHelp: strings.TrimSpace(`
The 'scaletail dns status' subcommand prints the current DNS status and
...
func formatDNSStatusText(data *jsonoutput.DNSStatusResult, all bool) string {
	...
	if data.TailscaleDNS {
		fmt.Fprintf(&sb, "Tailscale DNS: enabled.\n\nTailscale is configured to handle DNS queries on this device.\nRun 'scaletail set --accept-dns=false' to revert to your system default DNS resolver.\n")
	} else {
		fmt.Fprintf(&sb, "Tailscale DNS: disabled.\n\n(Run 'scaletail set --accept-dns=true' to start sending DNS queries to the Tailscale DNS resolver)\n")
	}
	fmt.Fprintf(&sb, "\n")
