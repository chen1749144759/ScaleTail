...
func init() {
	...
	clientupdateLatestScaleTailVersion.Set(func(track string) (string, error) {
		if track == "" {
			return clientupdate.LatestScaleTailVersion(clientupdate.CurrentTrack)
		}
		return clientupdate.LatestScaleTailVersion(track)
	})
}

var updateCmd = &ffcli.Command{
	Name:       "update",
	ShortUsage: "scaletail update",
	ShortHelp:  "Update ScaleTail to the latest/different version",
	Exec:       runUpdate,
	FlagSet: (func() *flag.FlagSet {
		...
		// the latest version of ScaleTail:
		//
...
func runUpdate(ctx context.Context, args []string) error {
	...
	if errors.Is(err, errors.ErrUnsupported) {
		return errors.New("The 'update' command is not supported on this platform; see https://scaletail.com/s/client-updates")
	}
	return err
}
...
func confirmUpdate(ver string) bool {
	if updateArgs.yes {
		fmt.Printf("Updating ScaleTail from %v to %v; --yes given, continuing without prompts.\n", version.Short(), ver)
		return true
	}
	...
	msg := fmt.Sprintf("This will update ScaleTail from %v to %v. Continue?", version.Short(), ver)
