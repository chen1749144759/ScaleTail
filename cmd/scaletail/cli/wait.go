...
With no arguments, this command will block until scaletaild is up, its backend is running,
and the Tailscale interface is up and has a Tailscale IP address assigned to it.

If running in userspace-networking mode, this command only waits for scaletaild and
...
To wait on a specific type of IP address, use 'scaletail ip' in combination with
the 'scaletail wait' command. For example, to wait for an IPv4 address:

    scaletail wait && scaletail ip --assert=<specific-IP-address>
...
can run it like 'scaletail wait && /path/to/service [...]' to ensure that Tailscale is ready
before the program starts.
`),

	ShortUsage: "scaletail wait",
	Exec:       runWait,
