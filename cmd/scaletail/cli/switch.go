...
var switchCmd = &ffcli.Command{
	...
	ShortUsage: strings.Join([]string{
		"scaletail switch <id>",
		"scaletail switch --list [--json]",
	}, "\n"),
	ShortHelp: "Switch to a different Tailscale account",
	LongHelp: `"scaletail switch" switches between logged in accounts. You can
...
This command is currently in alpha and may change in the future.`,
	...
	Subcommands: []*ffcli.Command{
		{
			Name:       "remove",
			ShortUsage: "scaletail switch remove <id>",
			ShortHelp:  "Remove a Tailscale account",
			LongHelp: `"scaletail switch remove" removes a Tailscale account from the
...
func switchProfile(ctx context.Context, args []string) error {
	...
	if switchArgs.json {
		outln("--json argument cannot be used with scaletail switch NAME")
		os.Exit(1)
	}
	if len(args) != 1 {
		outln("usage: scaletail switch NAME")
		os.Exit(1)
	}
...
func removeProfile(ctx context.Context, args []string) error {
	if len(args) != 1 {
		outln("usage: scaletail switch remove NAME")
		os.Exit(1)
	}
	cp, all, err := localClient.ProfileStatus(ctx)
	if err != nil {
