...
func jetKVMConfigureCmd() *ffcli.Command {
	...
	return &ffcli.Command{
		...
		ShortUsage: "scaletail configure jetkvm",
		ShortHelp:  "Configure JetKVM to run scaletaild at boot",
		LongHelp: strings.TrimSpace(`
This command configures the JetKVM host to run scaletaild at boot.
`),
...
# Start/stop scaletaild

case "$1" in
  start)
    /userdata/tailscale/scaletaild > /dev/null 2>&1 &
    ;;
  stop)
    killall scaletaild
    ;;
