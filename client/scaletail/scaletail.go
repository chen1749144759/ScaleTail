...
// Package scaletail contains a Go client for the ScaleTail control plane API.
//
// This package is only intended for internal and transitional use.
//
// Deprecated: this transitional package is retained only for internal tools.
package scaletail
...
// for now while this package is still treated as an internal transitional API.
...
// Client makes API calls to the ScaleTail control plane API server.
//
...
// Deprecated: this transitional package is retained only for internal tools.
type Client struct {
	// tailnet is the globally unique identifier for a ScaleTail network, such
...
// https://api.scaletail.com/api/v2/devices/5.
...
// would result in https://api.scaletail.com/api/v2/tailnet/example.com/policy/validate.
...
func NewClient(tailnet string, auth AuthMethod) *Client {
	return &Client{
		tailnet:   tailnet,
		auth:      auth,
		UserAgent: "scaletail-client-oss",
	}
}
...
// ErrResponse is the HTTP error returned by the ScaleTail server.
